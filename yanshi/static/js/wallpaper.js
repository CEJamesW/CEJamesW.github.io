// var _paq=window._paq=window._paq||[];_paq.push(["trackPageView"]),_paq.push(["enableLinkTracking"]),function(){var b,c,d,a="//stats.liumingye.cn/";_paq.push(["setTrackerUrl",a+"matomo.php"]),_paq.push(["setSiteId","2"]),b=document,c=b.createElement("script"),d=b.getElementsByTagName("script")[0],c.type="text/javascript",c.async=!0,c.src=a+"matomo.js",d.parentNode.insertBefore(c,d)}();

// var proxy_url = "https://bird.ioliu.cn/v2?url=";
var proxy_url = "https://cors.luckydesigner.workers.dev/?";
...
function loadData(types, newload) {
    ...
    switch (seting.types) {
        ...
        default:    // 加载来自360的壁纸
            ajax360Wal(seting.types, jigsaw.pageno, 20);
    }
}
...
function ajaxBingWal(start, count) {
    $.ajax({
        type: "GET",
        url: proxy_url + encodeURIComponent("https://cn.bing.com/HPImageArchive.aspx?format=js&idx="+start+"&n="+count),
        dataType : "json",
        success: function (jsonData) {
            let newHtml = "";
            if (isPC()) {
                newHtml += '<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/onepage-scroll@1.3.0/onepage-scroll.css">', downUrl = '';
...
function ajaxCiba(data) {
    $.ajax({
        type: "GET",
        url: proxy_url + "http://open.iciba.com/dsapi/", 
        dataType : "json",
        success: function (jsonData) {
...
function ajax360Tags() {
    $.ajax({
        type: "GET",
        url: proxy_url + "http://wp.birdpaper.com.cn/intf/getCategory",
        dataType : "json",
        success: function (jsonData) {
            var newHtml = '';
...
function ajax360WalSearch(cid, start, count, content) {
    ...
    $.ajax({
        type: "GET",
        url: proxy_url + "http://wp.birdpaper.com.cn/intf/search?content="+encodeURIComponent(content)+"&pageno="+start+"&count="+count,
        dataType : "json",
        success: function (jsonData) {
...
function ajax360Wal(cid, start, count) {
    ...
    if(cid == '360new'){
        ...
        url = "GetListByCategory?cids="+cid+"&";
    }
    $.ajax({
        type: "GET",
        url: proxy_url + "http://wp.birdpaper.com.cn/intf/"+url+"pageno="+start+"&count="+count,
        dataType : "json",
        success: function (jsonData) {
            for (var i = 0; i < jsonData.data.list.length; i++) {
