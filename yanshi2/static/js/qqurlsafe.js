...
var ajaxpos = 0;
//当前扫描位置
var apiurl = "http://api.bugscaner.com/qqurlsafe.py";
//api接口
var domainarr = {};
...
function mainstart() {
	...
	if (domaincount>300){
		toastr.warning("亲爱的用户,您最多只能查询300条网址,您输入的已经超过了300条,程序只查询前300条,后面的自动忽略")
	}else{
...
function appendlistbox(oldinfo,result) {
	...
	if (code == 3) {
		//查询成功
		totalLink ++
		var d = '<tr class="info"><td>'+startid+'</td><td class="domain">'+oldinfo+'</td><td class="status">已认证</td></tr>';
		returninfo.push(startid+"\t"+oldinfo+"\t"+"已认证");
		$("#insert").append(d);
	}
	else if(code == 2){
		totalLink ++
		var d = '<tr class="error"><td>'+startid+'</td><td class="domain">'+oldinfo+'</td><td class="status">已拦截</td></tr>';
		returninfo.push(startid+"\t"+oldinfo+"\t"+"已拦截");
		$("#insert").append(d);
	}
	else if(code == 1 ){
		totalLink ++
...
function runpoolover() {
    if (ajaxpos < domaincount) {
		...
		mainpause.attr("disabled","true");
		toastr.success("恭喜您域名拦截检测查询完毕 - 共耗时："+(+new Date()-starttime)+"ms,你现在可以点击导出按钮,导出到文本了！");
		}
		else{
			mainpause.attr("disabled","true");
			mainexport.removeAttr("disabled");
